...
これから、既存の軽量セルフホスト型 RSS リーダーの設計を参考にしながら、Rust で軽量なセルフホスト型 RSS リーダーを設計・実装してください。

これは特定実装の完全移植ではありません。
既存の RSS リーダーに見られる設計思想・機能構成・DB 設計・feed pull worker・UI/UX を参考にしつつ、Rust 学習と軽量セルフホストを目的に、まずは小さな MVP を作ることを重視します。

# 背景

参考にするのは、軽量なセルフホスト型 RSS リーダーで一般的に採用されている構成です。
...
ただし、今回最初に作るものは特定実装の完全互換ではありません。
...
既存の軽量セルフホスト型 RSS リーダーを参考にしてください。
...
軽量なセルフホスト構成として、1 プロセス内で以下を動かしてください。
